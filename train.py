...
CONFIG = {
    ...
    # Val/test gap widened 1.360→1.589. Seasonal distribution shift not addressable by episode-speed variation.
    ...
    # TransAtt decoder: replaces DirectHorizonHead with a Transformer-style decoder.
    # 6 horizon queries self-attend, then cross-attend to full T=24 encoder sequence.
    # Zero-init out_proj → starts at persistence baseline.
    ...
    'use_stae': False,      # STAEformer spatio-temporal adaptive embedding: (T=24, N=12, d=8) → proj → hidden_dim, added after input_proj+node_embed. Zero-init → zero start. 2,816 params.
...
def train_epoch(model, train_loader, optimizer, criterion, adj, config, teacher_forcing_ratio,
    ...
    use_moe_var_loss = config.get('use_moe_var_loss', False) and config.get('use_seg_moe', False)
    lambda_moe_var = float(config.get('lambda_moe_var', 0.02))
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        # MoE router variance loss: penalise low-variance (flat/uncertain) routing decisions.
        # Collects last_router_probs stored by each SegMoE instance during this forward pass.
        # L_var = -var(probs across 2 experts), minimising pushes router toward decisive [0,1] splits.
        if use_moe_var_loss:
            moe_probs = [m.last_router_probs for m in model.modules()
                         if hasattr(m, 'last_router_probs')]
            if moe_probs:
                all_probs = torch.cat([p.reshape(-1, 2) for p in moe_probs], dim=0)
                moe_var_loss = -all_probs.var(dim=-1).mean()
                loss = loss + lambda_moe_var * moe_var_loss
...
def train(config, trial=None):
    ...
    if model_type == 'graph_transformer':
        model = GraphTransformerModel(
            ...
            use_fft_features=config.get('use_fft_features', False),
